...
class TTK::ETrade::Options::Chains
  ...
  def option_expirations(symbol)
    unless @expirations_map[symbol]
      ...
      @expirations_map[symbol] = TTK::ETrade::Options::OptionExpirations.new.update(from_array: array)
    end
    ...
    unless @chains_map[symbol]
      ...
      Async do
        ...
        expirations.each do |expiration|
          @limiter.async do
            option_chain_response = chain_session.reload(symbol, expiration)
            accumulator << TTK::ETrade::Options::Chain.new.update(from_hash: option_chain_response)
          end
        end
        @barrier.wait
