...
class TTK::ETrade::Core::Quote
  ...
  def quote_timestamp
    Central_TZ.to_local(Time.at(quote['dateTimeUTC'] || 0))
  end
  ...
  class All < TTK::ETrade::Core::Quote
    ...
    def ask_time
      Central_TZ.to_local(Time.strptime(quote.dig(self.class::KEY, 'askTime'), "%H:%M:%S %z %m-%d-%Y"))
    end

    def bid_time
      Central_TZ.to_local(Time.strptime(quote.dig(self.class::KEY, 'bidTime'), "%H:%M:%S %z %m-%d-%Y"))
    end

    def last_Time
      Central_TZ.to_local(Time.at(quote.dig(self.class::KEY, 'timeOfLastTrade') || 0))
    end
